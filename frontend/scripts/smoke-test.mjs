const baseUrl = process.env.SMOKE_BASE_URL || 'http://127.0.0.1:3700';
...
const adminCookie = await login('admin@legal-compliance.local', 'admin123');
const managerCookie = await login('manager@legal-compliance.local', 'manager123');
const analystCookie = await login('analyst@legal-compliance.local', 'analyst123');

await expectJson('/api/dashboard', adminCookie);
await expectJson('/api/entities/matters', analystCookie);
await expectJson('/api/documents', analystCookie);
await expectStatus('/api/documents/upload', analystCookie, 405);

const records = await expectJson('/api/entities/matters', managerCookie);
const rowId = records.rows[0].id;

const approveResponse = await fetch(`${baseUrl}/api/entities/matters/approve`, {
  method: 'POST',
  headers: {
...
const forbiddenApprove = await fetch(`${baseUrl}/api/entities/matters/approve`, {
  method: 'POST',
  headers: {
